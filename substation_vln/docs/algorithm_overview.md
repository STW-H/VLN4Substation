...
当前项目采用“离线传统规划负责安全到达可观测区域，在线 VLN 负责局部主动观测”的分阶段架构。全局规划不预测机器人到达后的真实图像质量，也不依赖 VLN/VLM 产生底层安全路径。
...
3. 在三维离散点云中，根据观测距离和视线遮挡计算地面可观测区域
4. 将三维可观测结果映射到二维地图，并与硬安全空间求交
...
为了支持三维可观测区域计算，需要在轴矫正后的完整点云中定义巡视目标。当前将每个巡视目标简化为一个三维表面点，而不是预先固定唯一机器人停靠点。
...
```text
inspection targets
pointcloud visibility masks
safe observable region masks
task instruction templates
```

目标点必须位于需要观察的设备表面，例如表盘中心或状态指示区域，不能位于设备内部。机器人摄像头暂时假设为固定高度的全向相机，因此离线可观测区域计算不考虑相机 yaw、视场角和目标表面法向；真实图像是否合格及最终位姿调整由在线局部 VLN 负责。
...
标注程序已经加入 `narrow_space` 类别，并支持多边形、矩形和圆形标注；规划地图构建器尚未把它栅格化。下一步将新增 `narrow_space_mask` 和对应软风险代价，使狭窄空间保持可通行，但在路径搜索中承担可调惩罚。
...
```text
基于三维点云可见区域和上下文语义风险的变电站巡检区域目标规划
```

核心问题分为两个相互关联的部分：

```text
去哪里停：由三维点云中的目标距离和视线遮挡生成地面可观测区域，再用二维地图筛选安全停靠区域
如何到达：在统一硬安全空间内，根据道路、狭窄空间、障碍净空和任务环境上下文构造软代价
```

最终由区域目标 A* 在安全可观测区域中自动选择路径代价较低的终点。机器人到达该区域后的真实观测质量评价和短距离调整属于在线 VLN 阶段，不进入离线全局规划目标函数。
...
离线可观测区域计算只使用轴矫正后的完整点云，不依赖 3D Gaussian。当前作如下简化：
...
```text
observable_mask_i[row, col] = distance_valid and line_of_sight_valid
```

再与二维硬安全空间求交：

```text
safe_observable_mask_i = observable_mask_i & free_space_mask
```
...
传统 A* 的终点是固定栅格；本文将 `safe_observable_mask_i` 作为目标区域：

```text
终止条件：current cell in safe_observable_mask_i
...
局部 VLN 的在线成败不反向加入当前离线规划约束。后续可以单独研究失败恢复或全局重规划，但不属于第一版离线可观测区域算法。
...
这样才能分别量化三维可观测区域、区域目标搜索、语义代价和上下文自适应的收益。
...
构建完整点云三维体素占据地图
实现相机到目标点的三维体素射线遮挡检测
生成 observable_mask 和 safe_observable_mask
实现区域目标 A* 及目标区域距离启发图
